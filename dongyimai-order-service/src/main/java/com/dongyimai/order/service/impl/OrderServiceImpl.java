...
import com.dongyimai.mapper.TbPayLogMapper;
...
public class OrderServiceImpl implements OrderService {
	...
	public void add(TbOrder order) {
		...
		for (Cart cart : cartList) {
			...
			tbOrder.setStatus("0");//未付款
			tbOrder.setCreateTime(new Date());//创建日期
			tbOrder.setUpdateTime(new Date());//修改日期
			tbOrder.setReceiverAreaName(order.getReceiverAreaName());//送货地址
			tbOrder.setReceiverMobile(order.getReceiverMobile());//送货手机号
